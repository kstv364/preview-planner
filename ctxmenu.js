(function (root, factory) {
   ...
   * */
  Contextmenu.render = function (containerElement, data, option = {}) {
    /**
    ...
    if (!utils.isElement(containerElement)) {
      utils.debug.error(
        `The passed in container node parameter must be an element node.`
      );
      return undefined;
    }
    const profile = {
     ...
     * */
    manger.insertHtmlString(containerElement, data, profile);
    /**
  ...
  Contextmenu.hidden = function (containerElement) {
    if (!utils.isElement(containerElement)) {
      return;
    }
    manger.setContainerHidden(containerElement);
  ...
  Contextmenu.show = function (containerElement, option) {
    if (!utils.isElement(containerElement)) {
      return;
    }
    ...
    manger.setContainerPosition(containerElement, left, top);
  ...
  manger.bindEvent = function (
    ...
    containerElement.addEventListener(
      "mouseover",
      manger.containerElementMouseoverEvent,
      false
    );
    ...
    containerElement.addEventListener(
      "click",
      manger.containerElementClickEvent,
      false
    );
    ...
    containerElement.addEventListener(
      "contextmenu",
      manger.containerElementContextmenuEvent,
      false
    );
     ...
     * */
    const ctxmenuItemPointer = containerElement.querySelectorAll(
      `.ctxmenu-item-pointer`
    );
  ...
  manger.rmBindEvent = function (containerElement) {
  ...
  manger.insertHtmlString = function (containerElement, data, option) {
    containerElement.innerHTML = `${utils.createContextmenuWrapper(
      data,
      option
    )}`;
  };

  manger.setContainerPosition = function (containerElement, left, top) {
    containerElement.style.left = `${left}px`;
    containerElement.style.top = `${top}px`;
  };
  manger.setContainerHidden = function (containerElement) {
    containerElement.style.display = "none";
  };
  manger.setContainerShow = function (containerElement) {
    containerElement.style.display = "block";
  };
  manger.clearContainer = function (containerElement) {
    containerElement.innerHTML = ``;
  };

  manger.getContainerBoundingRect = function (containerElement) {
    /**
  ...
  manger.setExtendWrapperShow = function (extendWrapperElement, parentElement) {
    let left = 0;
    let top = 0;
    let isSetLeft = false;
    let isSetTop = false;
  ...
  manger.setExtendWrapperHidden = function (extendWrapperElement) {
    extendWrapperElement.style.display = `none`;
  };
      ...
      if (item.extend && item.extend.length) {
        htmlString += `
                    <li class="ctxmenu-item ${
                      item.separate ? "ctxmenu-item-separate" : ""
                    }" title="${item.name}">
                        <div class="ctxmenu-item-content ${
                          item.disabled ? "ctxmenu-item-content-disabled" : ""
                        }">
                            <div class="ctxmenu-item-tag"></div>
                            <div class="ctxmenu-item-text" data-cmd="${
                              cmkPrefix
                                ? cmkPrefix + option.hyphen + item.cmd
                                : item.cmd
                            }">${item.name}</div>
                        ...
                        <div class="ctxmenu-item-extend" style="z-index: ${option.zIndexBase++}; display: none;">
                            ${utils.createContextmenuWrapper(
                              item.extend,
                              option,
                              cmkPrefix
                                ? cmkPrefix + option.hyphen + item.cmd
                      ...
                      item.separate ? "ctxmenu-item-separate" : ""
                        ...
                        <div class="ctxmenu-item-content ${
                          item.disabled ? "ctxmenu-item-content-disabled" : ""
                            ...
                            <div class="ctxmenu-item-tag">${
                              item.selectedTag
                                ? '<div class="ctxmenu-item-tag-selected"></div>'
                                : ""
                            }</div>
                            <div class="ctxmenu-item-text" data-cmd="${
                              cmkPrefix
